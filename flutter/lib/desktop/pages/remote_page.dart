        ...
        children: [
          ...
          Stack(
            children: [
              ...
              // Bottom status bar + floating chat (always visible, even in fullscreen).
              Positioned(
                left: 0,
                right: 0,
                bottom: 0,
                child: _RemoteStatusBar(ffi: _ffi),
              ),
              Positioned(
                right: 16,
                bottom: 40,
                child: _FloatingChatButton(ffi: _ffi),
              ),
            ],
          ),
        ],
...
/// Bottom status bar of the remote window: "正在控制" peer name + live
/// connection stats (fps / latency / speed) + clock + encryption.
class _RemoteStatusBar extends StatefulWidget {
  final FFI ffi;
  const _RemoteStatusBar({Key? key, required this.ffi}) : super(key: key);

  @override
  State<_RemoteStatusBar> createState() => _RemoteStatusBarState();
}

class _RemoteStatusBarState extends State<_RemoteStatusBar> {
  Timer? _timer;
  String _clock = '';
  ...
  void initState() {
    super.initState();
    _refreshClock();
    _timer = Timer.periodic(const Duration(seconds: 1), (_) => _refreshClock());
  }
  ...
  Widget build(BuildContext context) {
    ...
    final name = pi.username.isNotEmpty ? pi.username : pi.hostname;
    final perf = widget.ffi.qualityMonitorModel.data;
    final fps = perf.fps ?? '';
    final delay = perf.delay ?? '';
    final speed = perf.speed ?? '';
    return Container(
      height: 30,
      ...
      child: Row(
        children: [
          Expanded(
            child: Text(
              '正在控制：$name${pi.platform.isNotEmpty ? ' (${pi.platform})' : ''}',
...
/// Floating chat entry that stays on top of the remote canvas (including
/// fullscreen) so the user can jump back to the conversation at any time.
class _FloatingChatButton extends StatelessWidget {
  final FFI ffi;
  const _FloatingChatButton({Key? key, required this.ffi}) : super(key: key);
